...
# FQE train and evaluation for DT model
win_return=$1
...
for policy_seed in "${policy_seeds[@]}"; do
    echo "new fqe-dt training instance start."
    python fqe_tactics_main.py \
        ...
        --n_layer 3 \
        --n_head 2 \
        --use_win_return $win_return \
        --policy_path "policy_models_save_final" \
        --checkpoints_path "fqe_models_save_final" \
        > fqe_logs/train_fqe_dt_w${win_return}_s${policy_seed}.log 2>&1 &
    wait
done

echo "fqe-dt training done!"