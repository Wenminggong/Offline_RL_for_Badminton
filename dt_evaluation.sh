...
# DT hyper-parameter and random seed
                    ...
                    for n_head in "${n_heads[@]}"; do
                        for policy_seed in "${policy_seeds[@]}"; do
                            ...
                            python tactics_model_evaluation.py \
                                ...
                                --n_layer $n_layer \
                                --n_head $n_head \
                                > evaluation_logs/eval_dt_reward_${filter_type}_b${policy_batch_size}_h${policy_hidden_size}_e${policy_embed_size}_nl${n_layer}_nh${n_head}_s${policy_seed}.log 2>&1 &
                            wait
                        done
                    done
                done
