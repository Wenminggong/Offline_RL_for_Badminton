...
# OPE hyper-parameter and random seed for DT model
batch_sizes=(128)
                    ...
                    for policy_seed in "${seeds[@]}"; do
                        for fqe_n_layer in "${fqe_n_hidden_layers[@]}"; do
                            for fqe_embed_size in "${fqe_embed_sizes[@]}"; do
                                echo "new ope-dt training instance start."
                                python ope_tactics_main.py \
                                    ...
                                    > ope_logs/train_fqe_dt_fn${fqe_n_layer}_fe${fqe_embed_size}_b${batch_size}_h${hidden_size}_e${embed_size}_nl${n_layer}_nh${n_head}_s${policy_seed}_new.log 2>&1 &
                                wait
                            done
                        done
                    done
