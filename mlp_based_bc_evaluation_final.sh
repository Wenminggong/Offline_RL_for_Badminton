...
# evaluating the average reward of MLP-BC used to generate the dataset by using reward model.
...
for filter_type in "${filter_types[@]}"; do
    for policy_seed in "${policy_seeds[@]}"; do
        ...
        python tactics_model_evaluation.py \
            ...
            > evaluation_logs/eval_mlp_bc_reward_${filter_type}_s${policy_seed}.log 2>&1 &
        wait
    done
done
